...
export async function connectContinueCommand(): Promise<void> {
  ...
  if (result.ok) {
    ...
    const manualConfig =
      pick.label === 'Continue.dev'
        ? null
        : `Base URL: http://localhost:${port}/v1\nAPI Key: devflow-local`;
    if (manualConfig) {
      msg += ` If it doesn't appear, add Base URL and API Key manually in the tool's settings.`;
    }
    const actions: string[] = ['Reload Window', 'Open Dashboard'];
    if (manualConfig) actions.push('Copy Manual Config');
    ...
    if (action === 'Reload Window') {
      ...
      vscode.window.showInformationMessage(
        'Copied. Paste Base URL and API Key into your AI tool settings.'
      );
    }
  } else {
